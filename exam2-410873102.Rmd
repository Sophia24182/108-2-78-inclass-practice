...
factor(tpe_highSchools, levels = new_levels) -> tpe_highSchools
...
```{r ans22}
data_note <- data228$附註 == "表冊名稱:臺灣省二二八事變正法及死亡人犯名冊"
data_num <- data228$檔號 == "A803000000A/0036/340.2/5502.3/19/007"
victims <- data228$個人名稱[data_note & data_num]
class(victims)
...
```{r ans23}
event <- table(data228$個人名稱)
pick_name <- names(event) %in% victims
number <- event[pick_name]
documentCount_eachVictim <- sort( as.integer(number), decreasing = T)
names(documentCount_eachVictim) <- names(number)
...
```{r ans31}
grade <- str_extract(courseStructure$應修系級, "([:graph:]+組[1-4]|[:graph:]+系[1-4])")
majorGradeCombos <- unique(grade)
majorGradeCombos
```
...
```{r ans32}
pick_103 <- courseStructure$學年 == "103"
pick_eco <- str_detect(courseStructure$應修系級, "經濟系3")
econ101junior <- levels(factor(courseStructure$科目名稱[pick_103 & pick_eco]))
