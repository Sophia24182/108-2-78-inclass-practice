...
T_F <- c(table_countsByArticle >= max_number)
mostViolatedArticle <- table_countsByArticle[T_F]
...
```{r ans14}
violatedContent <- table(badEmployers$lawContent)
mostviolatedContent <- violatedContent == max(violatedContent)
mostViolatedArticle_content <- violatedContent[mostviolatedContent]
mostViolatedArticle_content
mostViolatedArticle_content <- as.character(mostViolatedArticle_content)
...
max(levels(factor(month(badEmployers$dt)))) -> popularSentenceMonth
...
factor是從1開始的整數，沒有0喔。
```{r ans21}
chinese <- levels(as.factor(table[["國文"]]))
chinese
```
...
```{r ans22}
T_Ftable_for_student <- str_detect(students10$志願, "北大")
ntpu_namelist <- students10$姓名[T_Ftable_for_student]
ntpu_namelist
```
...
logical儲存的本質是甚麼?
```{r ans23}
if_select_ntpu_econ <- str_detect(students10$志願, "北大經濟")
if_select_ntpu_econ
type1_list <- students10$國文 >= 12
type1_list
type2_list <- (students10$英文+ students10$數學) >= 26
type2_list
total_list <- if_select_ntpu_econ + type1_list + type2_list
ntpu_econtpu_eco <- as.character(students10$姓名[total_list >= 3])
ntpu_econtpu_eco
```
...
```{r ans24}
table_for_congrat <- students10$姓名 %in% namelist
table_for_congrat
part1 <- c("恭喜")
part_name <- ntpu_eco[table_for_congrat]
part2 <- c("同學錄取國立臺北大學經濟系")
congrats <- str_c(part1, part_name, part2)
congrats
```
